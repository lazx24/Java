...
public abstract class URLUtil {
	...
	return addQueryString(url, new String[] { name },
		new Object[] { value });
    }
     ...
     * @return 拼接后的URL
     */
    public static String addQueryString(String url, String[] names,
	    Object[] values) {
	...
	return servletPath.substring(
		servletPath.lastIndexOf(SEPARATOR_SIGN) + 1,
		servletPath.lastIndexOf(POINT_SIGN));
    }
	...
	return interrogationIndex == -1 ? url.substring(pointIndex + 1) : url
		.substring(pointIndex + 1, interrogationIndex);
    }
	...
	return servletPath
		.substring(0, servletPath.lastIndexOf(SEPARATOR_SIGN));
    }
     ...
     * @throws IOException
     */
    public static InputStream doGet(String pageURL) throws IOException {
	...
	return !ValidUtil.isEmpty(url) && !ValidUtil.isEmpty(baseURL)
     ...
     * @return 拼接后的查询字符串
     */
    private static StringBuilder appendParameter(StringBuilder queryString,
	...
	return doPost(url, DEFAULT_TIME_OUT, "");
    }

    public static InputStream doPost(String url, String requestData) {
	return doPost(url, DEFAULT_TIME_OUT, requestData);
    }
     ...
     * @return inputStream
     */
    public static InputStream doPost(String url, int TIME_OUT,
	    String requestData) {
	URL postUrl;
	try {
	    postUrl = new URL(url);
	    ...
	    if (!ValidUtil.isEmpty(requestData)) {
	...
	url = addQueryString(url, "tel", mobile);
	url = url + "/";
	url = ignoreLastRightSlash(url);
	System.out.println("清除最后符号/:" + url);
	System.out.println("获取URL的后缀名:" + getExtension(url));
	InputStream in = doPost(url);
	// System.out.println("请求URL返回String:"+readContent(url));
