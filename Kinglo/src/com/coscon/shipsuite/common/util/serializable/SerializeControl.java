package com.coscon.shipsuite.common.util.serializable;
...
public class SerializeControl {
    /**
     * 内部类，用于测试控制对象的序列化和反序列化。 被序列化的对象必须实现Externalizable接口，该接口有2个方法：
     * writeExternal方法、readExternal方法。
     * 当序列化对象的时候，writeExternal方法会自动执行，所有的对象写入操作由该方法控制。
     * 反序列化对象的时候，readExternal方法会自动执行，如果想访问writeExternal写入的内容， 也只能在该方法里编写恢复对象的代码。
     */
